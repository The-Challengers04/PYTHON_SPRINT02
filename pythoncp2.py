#funções utilizadas:
#realizando o login:
def criarcadastro(): #função
    nome = input('Digite seu nome ') #pede nome
    sobrenome = input('Digite seu sobrenome ') #pede sobrenome
    email = input('Digite seu email ') #pede email
    senha = input('Digite sua senha ') #pede senha
    logins = open('logins.txt','a') #abre um arquivo txt e coloca as informações do usuário
    logins.write(f'-Nome: {nome} -Sobrenome: {sobrenome} -Email: {email} -senha: {senha} \n') #escreve no arquivo as informações coletadas

#fazendo a review:
def fazerreview():
    nomeEstabelecimento = input('Digite o nome do estabelecimento') #pede nome do estabelecimento
    locEstabelecimento = input('Digiteo endereço do estabelecimento') #pede endereço do estabelecimento
    notaEstabelecimento = int(input(f'Digite uma nota, de 0 a 5, para a acessibilidade dentro do estabelecimento {nomeEstabelecimento}')) #nota para o estabelecimento
...
def postcomunidade():
    ...
    confirmação = input(f'Você deseja mesmo postar isso? Digite s para sim e n para não {post}') #confirma se o usuário quer mesmo fazer o post
    if confirmação.lower() == 's':
        print('Postagem concluída')
    elif confirmação.lower() == 'n':
        reconfirma = input('Reescreva sua postagem ou aperte 1 para sair') #loop para o usuário reescrever o post, ou apertar 1 e apagar o post
        if reconfirma != 1:
            reconfirma = post #o novo post salvo na variável post
            print('Postagem concluída')
        else:
            print('Post deletado') #se o usuário digitar 1, o post é deletado
